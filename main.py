...
opts, args = getopt.getopt(argv, "s:f:d:",["starting_point=", "step_forward=","destination="])
...
destination = 'wire_up.json' if destination is None else destination

data = read_data_json(filename = destination)
...
for key, value in data.items():
    ...
    if old_number_of_placeholder >= starting_point:
        new_number_of_placeholder = old_number_of_placeholder + step_forward
